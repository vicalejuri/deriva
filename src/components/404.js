...
export let NotFoundComponent = React.createClass({
  showPolygons(){
    let polygons = _.map( this.refs.svg.childNodes, (g_node) => { return g_node.childNodes; } );
    let all      = _.union( polygons[0], polygons[1] , polygons[2] );

    _(all).forEach( (el) => {
        setTimeout(function() {
           el.setAttribute('class', 'processed');
        }, Math.random() * 2000);
    });
  },
...
<svg ref="svg" width="620px" height="250px" viewBox="0 0 620 250">
  <g>
    <polygon ref="polygon" fill="none" stroke="#63C4AE" stroke-miterlimit="10" points="515.432,111.35 520.317,95.257 522.903,85.487
      491.58,100.43 	"/>
  ...
  <g>
    ...
    <polygon fill="none" stroke="#63C4AE" stroke-miterlimit="ac10" points="297.356,190.52 284.999,205.463 290.459,211.498 	"/>
